        ...
        const remote = async () => {
            new Promise(async (resolve,reject)=>{
                try {
                    ...
                    remoteConfig()
                        .setDefaults({
                            'uk_remote_config': JSON.stringify(DefaultRemoteConfig),
                        ...
                        .then(async (fetchedRemotely) => {
                            if (fetchedRemotely) {
                                ...
                                let config1 = config['uk_remote_config']
                                console.log('[config]',config1)
                                setRemoteConfig(JSON.parse(config1._value));
                                await AsyncStorage.setItem('uk_remote_config',config1._value);
                                Store.setRemoteConfig(config1._value ? JSON.parse(config1._value) : {})
                                resolve()
                            } else {
                                AsyncStorage.getItem('uk_remote_config').then((conf)=>{
                                    if(conf !== null){
